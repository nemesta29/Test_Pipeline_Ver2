...
class RockDataset(utils.Dataset):
    ...
    def load_rock(self, dataset_dir,subset):
		...
		#if loading training images
		if subset == 'train':
			#loading augmented images
			annotations = json.load(open(os.path.join(dataset_dir, "aug/via_region_data.json")))
			annotations = list(annotations.values())
			...
			#loading new images		
			annotations = json.load(open(os.path.join(dataset_dir, "new/via_region_data.json")))
			annotations = list(annotations.values())
